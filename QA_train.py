...
def main():
        ...
        for dict in raw_datasets["train"]:
            for idx in range(4):
                if dict["paragraphs"][idx] == dict["relevant"]:
                    train_context.append(context[dict["relevant"]])
        raw_datasets["train"] = raw_datasets["train"].add_column("context", train_context)

        validation_context = []
        for dict in raw_datasets["validation"]:
            for idx in range(4):
                if dict["paragraphs"][idx] == dict["relevant"]:
                    validation_context.append(context[dict["relevant"]])
        raw_datasets["validation"] = raw_datasets["validation"].add_column("context", validation_context)

    # See more about loading any type of standard or custom dataset (from files, python dict, pandas DataFrame, etc) at
    # https://huggingface.co/docs/datasets/loading_datasets.html.

    # Load prevalidationed model and tokenizer
    #
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        if args.version_2_with_negative:
            formatted_predictions = [v for _, v in predictions.items()
            ]
        else:
            formatted_predictions = [v for _, v in predictions.items()]

        references = [ex[answer_column_name]["text"] for ex in examples]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        plot_metric.append(eval_metric["exact_match"])
